...
def train(model, criterion, optimizer, dataset, max_epoch, batch_size, disp_freq):
    ...
    with tf.Session(config=config) as sess:
        ...
        train_data = (
            tf.data.Dataset.zip((tmp1, tmp2)).repeat(max_epoch).batch(batch_size)
        )
        ...
        valid_data = (
            tf.data.Dataset.zip((tmp3, tmp4)).repeat(max_epoch).batch(batch_size)
        )
        ...
        for epoch in range(max_epoch):
            batch_train_loss, batch_train_acc = train_one_epoch(
                model,
                criterion,
                optimizer,
                train_get_next,
            ...
            batch_val_loss, batch_val_acc = validate(
                model, criterion, valid_get_next, batch_size, sess
            )
            ...
            print(
                "Epoch [{}]\t Average validation loss {:.4f}\t Average validation accuracy {:.4f}".format(
